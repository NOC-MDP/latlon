...
See latlong_utm.h for a history of this code.


26-May-00 tc@DinkumSoftware.com Initial
27-Dec-01 tc@DinkumSoftware.com Port to linux
08-Dec-03 pfurey@DinkumSoftware.com Cast LongTemp to be float when computing
                                    ZoneNumber to avoid round off error on
                                    UTM boundaries in LLtoUTM().

Table of contents
    LLtoUTM    lat/long to UTM
    UTMtoLL    UTM to lat/long

    private, i.e. static
    UTMLetterDesignator    lat to UTM zone letter
*/
...
Converts "Lat" "Long" to UTM and outputs:
    "UTMNorthing, UTMEasting, UTMZone

UTMZone should be at least "UTMZone_max_strlength" chars long
An example UTMZone output string is: 62W
...
26-May-00 tc@DinkumSoftware.com Initial, see latlong_utm.h for credits/history
08-Dec-03 pfurey@DinkumSoftware.com Cast LongTemp to be float when computing
                                    ZoneNumber to avoid round off error on
                                    UTM boundaries.
*/
...
//Make sure the longitude is between -180.00 .. 179.9
	...
	int    ZoneNumber;
        ...
        // cast LongTemp to float to remove potential round off error
	ZoneNumber = ((int)(((float)LongTemp + 180)/6)) + 1;
...
Converts "UTMNorthing/UTMEasting/UTMZone" to "Lat/Long".

East Longitudes are positive, West longitudes are negative. 
North latitudes are positive, South latitudes are negative
Lat and Long are in decimal degrees. 

Orignally Written by Chuck Gantz- chuck.gantz@globalstar.com
26-May-00 tc@DinkumSoftware.com Initial, see latlong_utm.h for credits/history
